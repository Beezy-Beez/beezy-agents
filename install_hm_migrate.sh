#!/usr/bin/env bash
# install_hm_migrate.sh
# Drops scripts/migrate_hive_mind_page.py into the beezy-agents-ingestion project.
# Strips Zipify wrappers, swaps subscribe section, bumps 17px→18px, drops template suffix.
...
cat > scripts/migrate_hive_mind_page.py <<'PYEOF'
"""
...
def bump_fonts(html: str) -> int:
    """Replace font-size:17px with font-size:18px. Returns count of replacements."""
    count = html.count("font-size:17px")
    return count


def apply_font_bump(html: str) -> str:
    return html.replace("font-size:17px", "font-size:18px")
...
def migrate(handle=None, page_id=None, dry_run=False):
    ...
    bump_count = bump_fonts(new_inner)
    bumped = apply_font_bump(new_inner)
    print(f"Font bumps:      {bump_count} (17px → 18px)")
...
echo
echo "Installed scripts/migrate_hive_mind_page.py"
echo
echo "Next steps:"
echo "  1. List all candidate pages:"
echo "       python -m scripts.migrate_hive_mind_page --list"
echo
echo "  2. Dry-run on Issue 12 (riskiest — smallest fonts, oldest format):"
echo "       python -m scripts.migrate_hive_mind_page --handle breathing-vagus-nerve-sleep-technique --dry-run"
echo
echo "  3. If output looks right, apply live:"
echo "       python -m scripts.migrate_hive_mind_page --handle breathing-vagus-nerve-sleep-technique"
echo
echo "  4. Visit the page, confirm visual match with Issue 15 styling."
echo
echo "  5. Repeat steps 2-4 for each remaining Hive Mind issue."
