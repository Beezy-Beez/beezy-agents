...
- `dashboard/` — Next.js operator dashboard (premium "clean light SaaS" rebuild; 7 routes incl. business overview; live data + inline editing; deploy to Vercel)

### Not built / known gaps
- **Next.js dashboard** (`dashboard/`) — premium operator product, fully rebuilt May 2026, compiles clean (`npm run build` passes, TS strict). Design system: Inter, slate/indigo light SaaS, recharts, shared component library (`components/`: Card, StatCard, Badge, Button, ActionButton, DataTable, Drawer, Field, charts, Toast, States). 7 routes: Overview (store-wide Shopify revenue vs $150k goal, attribution split, pace marker, trend), Calendar (plan-vs-actual table + inline slot add/edit/delete Drawer + approve), Audiences (RPR bars + burn/unburn), Analytics (revenue + pacing-trajectory charts, top performers, learning loop), Flows (defensive flow health + deliverability), Content (Hive Mind issue editing, SEO-topic CRUD, episodes), System (cron sentinels, runs, env, ops triggers, Slack command). All data live from Klaviyo/Shopify/Postgres — no invented numbers (§0). Backend extras in `app/main.py`: read `/api/data/business`, `/api/data/pacing-history`, `/api/data/deliverability`; write `/api/calendar/slot` (POST/PATCH/DELETE), `/api/content/issue` (PATCH), `/api/content/seo-topic` (POST/PATCH/DELETE); helpers `_store_revenue`/`_pacing_history`/`_deliverability` in `app/dashboard.py`.
- **Dashboard is served by the Replit FastAPI server itself** — not a separate deploy. `dashboard/next.config.js` uses `output: 'export'`, `basePath: '/dashboard'`, `trailingSlash: true`. The static build (`dashboard/out/`, **committed** — Autoscale does not run `npm run build`) is mounted at `/dashboard` via `StaticFiles` in `app/main.py`. API is same-origin (relative `/api/*`); `NEXT_PUBLIC_API_URL` stays unset (dev auto-targets :8080). The legacy FastAPI HTML dashboard moved to **`/dashboard-classic`** (fallback). **After any `dashboard/` source change you must rebuild and recommit `dashboard/out/`:** `cd dashboard && npm run build`. Vercel is still possible (set `NEXT_PUBLIC_API_URL`) but not the primary path.
